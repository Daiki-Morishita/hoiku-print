...
export function Header({ materialCount = 555 }: { materialCount?: number }) {
  ...
  return (
    <>
      {/* Top thin bar */}
      <div className="hidden md:block bg-[#2a2a2a] text-[#dcdcdc] text-[11px]">
        <div className="max-w-[1280px] mx-auto px-6 flex justify-between items-center h-7">
          <div className="flex items-center gap-4">
            <span>保育士・幼稚園教諭の先生方へ</span>
            <span className="text-[#888]">／</span>
            <span>100%無料・会員登録不要</span>
            <span className="text-[#888]">／</span>
            <span>毎日更新</span>
          </div>
          <div className="flex items-center gap-4">
      ...
      <header className="sticky top-0 z-50 bg-background border-b border-border">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-[auto_1fr_auto] md:grid-cols-[220px_1fr_220px] gap-3 md:gap-8 items-center h-16 md:h-20">
            ...
            {/* Search bar (desktop) */}
            <form action="/materials" method="get" className="hidden md:flex border-[1.5px] border-foreground rounded overflow-hidden bg-white shadow-sm">
              <input
                name="search"
                type="text"
                value={searchValue}
            ...
            <div className="hidden md:block text-right">
              <div className="font-mincho text-[24px] font-black text-primary leading-none">{materialCount}</div>
              <div className="text-[11px] text-muted-foreground mt-0.5">点の無料教材</div>
            </div>
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-border bg-background">
            <div className="px-4 py-3">
              <form action="/materials" method="get" className="flex border-[1.5px] border-foreground rounded overflow-hidden bg-white mb-3">
                <input
                  name="search"
                  type="text"
                  placeholder="ぬりえを検索"
