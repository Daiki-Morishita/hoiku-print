...
```
背景は必ず純白 #FFFFFF
幼児向け塗り絵スタイル（塗りつぶしなし）。かわいくデフォルメされたイラスト調・丸みを強調したゆるいタッチ
輪郭線は太くはっきりと描く。内部の細部（質感・葉脈など）は少量の細い線でやさしく表現してよい
A4横長・高画質
...
### 比較UI（どちらの画像が良いですか？）対策
- プロンプトに「画像は1枚だけ」と明記して比較提案を抑制
- それでも比較UIが出た場合: スクリプトが自動で左側（A）を選択して続行
- 実装: `wait_for_image()` 内の比較UI検知 → 自動クリック処理
...
### スタイル方針
- **幼児向けデフォルメ**: 純粋な幾何学線画でも写実でもなく、丸みのあるかわいいイラスト調
- 花びらの質感・葉脈など内部細部は少量の細い線でOK（過剰なディテールはNG）
